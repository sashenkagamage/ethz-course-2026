...
A: with the increase of a the lemniscate becomes wider, so can be unreachable for the end effector 

1. What can happen if you change the dt parameter in IK?

A: dt is the timestep for each iteration. the maximum number of iterations defined is 2000. when I set the dt to be 2, the model becomes unstable and cannot converge the error to be < 5e-3. When I change the dt to be 0.001 the model performs very slowly, and still cannot converge within the limit. 

1. We implemented a simple numerical IK solver. What are the advantages and disadvantages compared to an analytical IK solver?

A: in analytical IK solvers we determine the joint angles based on algebraic and geometric equations, whereas in numerical solvers we guess the initial position and iteratively minimize the error from the target to the position. 

since numerical IK solvers minimizes the error the model is highly generalizable, but the computation is slow. the analytical solvers are fast and efficient, while they are specific to a certain DOF robots

1. What are the limits of our IK solver compared to state-of-the-art IK solvers?

compared to pinocchio and mink, this IK solver lacks joint and collision constraint handling, multi-objective optimization, robust singularity mitigation, and the highly optimized real-time execution speeds of compiled libraries.
...
[https://github.com/kevinzakka/mink](https://github.com/kevinzakka/mink)
...
A: it goes crazy fast. trying to track the error aggresively, probably will overshoot or become unstable irl

1. How does $K_D$ mitigate the effect you saw above when increasing $K_P$?

when i increase kp = 800 and kd to be 0.1, the kd tries to damp the overshoot. 

1. In what scenarios is a non-zero $K_I$ needed for the controller to perform well?

if there's a constant error or a steady-state error the integral term will remove the sum of the errors. 
...
3. **Action Processing:** The environment receives the normalized action and converts it into joint position targets within the robot's physical limits.
  ```python
  process_action(action: np.ndarray, jnt_range: np.ndarray)
  ```
...
Then open the TensorBoard at the provided link ([http://localhost:6006](http://localhost:6006)) in your browser.
...
**Observation:** The RL policy usually tracks the lemniscate approximately, instead of reaching each keypoint exactly. 
