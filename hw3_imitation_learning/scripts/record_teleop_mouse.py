"""Mouse-drag teleop for SO-100 demos (OpenCV window).
...
class _OpenCvMouseMixin:
    """OpenCV window with left-drag EE control (overrides Cv2 ``run``)."""

    _teleop_camera: str = "top"  # mostly top-down → intuitive XY dragging
    ...
    def _setup_mouse_state(self) -> None:
        ...
        self._cam_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_CAMERA, self._teleop_camera
        )
        if self._cam_id == -1:
            # fall back to angle if top is missing
            self._teleop_camera = "angle"
            self._cam_id = mujoco.mj_name2id(
                self.model, mujoco.mjtObj.mjOBJ_CAMERA, self._teleop_camera
            )

    def _camera_axes(self) -> tuple[np.ndarray, np.ndarray]:
        """Return (right, up) unit vectors of the teleop camera in world frame."""
        ...
        right = R[:, 0].copy()
        up = R[:, 1].copy()
        # Prefer motion in the horizontal plane for left-drag.
        right[2] = 0.0
        up[2] = 0.0
    ...
    def _apply_xy_delta(self, dx_pix: float, dy_pix: float) -> None:
        ...
        # OpenCV y grows downward → flip for camera "up"
    ...
    def _on_mouse(self, event: int, x: int, y: int, flags: int, _param) -> None:
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            # flags > 0 → scroll up on most backends
            direction = 1.0 if flags > 0 else -1.0
            self._apply_z_delta(direction * _Z_WHEEL)

    def _render_teleop_view(self) -> np.ndarray:
        img = self._render_bgr(self._teleop_camera)
        # Draw a marker at the projected EE so the user knows what they control.
        ee = self.data.site_xpos[self.ee_site_id]
        # Approximate: just overlay status; projection is optional.
        status = (
        ...
        hint = "L-drag move | wheel/R-drag height | Space rec | Enter end | R reset | J/N grip | Esc quit"
        cv2.putText(
    ...
    def _key_to_named_action(self, k_raw: int, k_ascii: int) -> str | None:
        """Map OpenCV key to an action name (keymap + fixed fallbacks)."""
        action = self._key_to_action.get(k_raw)
        if action is not None:
            return action
        # Fallbacks (in case keymap uses different codes than this machine)
        fixed_ascii = {
            ord(" "): "record",
            13: "end_episode",
            10: "end_episode",
    ...
    def _dispatch_named(self, action: str) -> None:
        # Reuse subclass handlers via keymap raw code when present.
        ...
        # Minimal session fallbacks if keymap lacks the binding.
        if action == "escape":
            if self.recording:
    ...
    def run(self) -> None:
        ...
        print(
            "\n=== Mouse teleop (OpenCV) ===\n"
            f"View: '{self._teleop_camera}' camera\n"
        ...
        try:
            while self.running:
                ...
                if k_raw != -1:
                    ...
                    if action is not None:
                        # Ignore EE move/rotate key actions — mouse owns those.
                        if action.startswith(("move_", "rot_")):
                            pass
                        else:
                ...
                cv2.imshow(self.window_name, self._render_teleop_view())
        finally:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="top",
        choices=["top", "angle", "left_wrist"],
        help="Camera used for the mouse teleop view (default: top).",
    )
    ...
    if args.multicube:
        ...
        rec = MouseMulticubeRecorder(
            xml_path=xml_path,
            out_zarr=out,
            control_hz=10.0,
            seed=args.seed,
        )
        rec._teleop_camera = args.camera
        rec.run()
        return
